...
if (!addressMap['discovery.olv.nintendo.net']) {
	if (!process.env.SSSL_DNS_DEFAULT_ADDRESS) {
		console.log(colors.bgRed('Mapping for discovery.olv.nintendo.net not found and no default address set. Set either SSSL_DNS_DEFAULT_ADDRESS or SSSL_DNS_MAP_discovery.olv.nintendo.net'));
		process.exit();
	}

	addressMap['discovery.olv.nintendo.net'] = process.env.SSSL_DNS_DEFAULT_ADDRESS;
}
